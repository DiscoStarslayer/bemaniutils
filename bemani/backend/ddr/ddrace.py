...
class DDRAce(
    ...
    def __handle_userload(self, userid: Optional[UserID], requestdata: Node, response: Node) -> None:
        ...
        grade.add_child(Node.u32("single_grade", 0))
        grade.add_child(Node.u32("dougle_grade", 0))
        response.add_child(grade)

        # Ref ID?
        ref_id_locked = Node.bool("is_refid_locked", False)
        response.add_child(ref_id_locked)

        # Event Data Count
        event_data_count = Node.s16("eventdata_count_all", 1)
        response.add_child(event_data_count)
